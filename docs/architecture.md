...
Action dispatch requires two inclusive gates: the selected candidate probability
must reach its operation threshold and the Choice confidence must reach the
overall threshold. Defaults are 0.65/read, 0.75/search, 0.80/graph and 0.50
confidence; configure them with `SAGE_JEV_READ_PROBABILITY_THRESHOLD`,
`SAGE_JEV_SEARCH_PROBABILITY_THRESHOLD`, `SAGE_JEV_GRAPH_PROBABILITY_THRESHOLD`
and `SAGE_JEV_ACTION_CONFIDENCE_THRESHOLD`. Values are finite numbers from zero
through one. They control dispatch only: they neither change Jev's distribution
nor avoid the request. A failed gate records observed and required values and
hands control back to the Solver. Excerpt Score acceptance is independent.
