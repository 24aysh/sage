...
from sage.harness.retrieval.parsing import CodeParser
...
def test_obsolete_partial_schema_requires_a_rebuild(tmp_path: Path) -> None:
    database = tmp_path / "obsolete.sqlite3"
    with sqlite3.connect(database) as connection:
        connection.execute("PRAGMA user_version=3")

    with pytest.raises(ValueError, match="obsolete; move or remove the derived index"):
        GraphStore(database)
