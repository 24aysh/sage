...
def test_cli_publishes_only_accepted_files_and_discard_counts(fixture_repo, built_memory, tmp_path, monkeypatch, capsys, failure):
    ...
    monkeypatch.setattr("sage.cli.memory.JevSettings.from_env", lambda: settings)
    monkeypatch.setattr("sage.cli.memory.build_legion_memory_service", lambda: service)
    monkeypatch.setattr("sage.cli.memory.build_relevance_filter",
                        lambda config: RelevanceFilter(settings=config, provider=provider))
    exit_code = main(["memory", "retrieve", "--repo", str(fixture_repo), "--issue-file", str(issue),
                      "--memory-file", str(database)])
...
def test_solve_filters_before_prompt_and_preserves_usage_and_repair_locators(fixture_repo, built_memory, tmp_path):
    service, database = built_memory
    build = service.build_or_update_graph_tool(repo_root=fixture_repo, memory_file=database)
    issue = "Fix `Worker.run` using `helper`."
    retrieval = service.retrieve_issue_context(issue_text=issue, repo_root=fixture_repo, memory_file=database)
    memory = MemorySession(service, fixture_repo, database, database, build, retrieval)
    ...
    context = SolveContext(prepared_run=prepared, repository=SimpleNamespace(get_changed_files=lambda: ()), settings=config,
                           artifacts=artifacts, memory=memory)
    provider = Provider()

    async def inspect(**kwargs):
        assert "service.py" in kwargs["message"]
        assert "tests/test_service.py" not in kwargs["message"]
        assert not kwargs["context"].memory.enrichment_enabled
        assert kwargs["context"].memory.enrich(tool_name="read_file", available_chars=2000, path="app.py") == ""
        repair = memory.begin_session(initial_visible=False)
        assert "service.py" in repair and "tests/test_service.py" not in repair
        memory.invalidate("service.py")
        assert memory.begin_session(initial_visible=False) == ""
