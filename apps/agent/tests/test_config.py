...
def test_jev_is_opt_in_and_credentials_are_redacted():
    ...
    assert (defaults.score_threshold, defaults.confidence_threshold) == (2, .5)
    ...
    for key, value in [("SAGE_JEV_NAVIGATION_POLICY", "actions"),
                       ("SAGE_JEV_MAX_FOLLOWUP_ACTIONS", "1"),
                       ("SAGE_JEV_TIMEOUT_SECONDS", "3"), ("SAGE_JEV_LOG_INPUT", "invalid"),
                       ("SAGE_JEV_RELEVANCE_SCORE_THRESHOLD", "3.1"),
                       ("SAGE_JEV_RELEVANCE_CONFIDENCE_THRESHOLD", "nan")]:
        with pytest.raises(ConfigurationError):
            JevSettings.from_env({key: value, "SAGE_JEV_NAVIGATION_MODE": "on", "TYPESAFE_API_KEY": "private-key"})
    enabled = JevSettings.from_env({"SAGE_JEV_NAVIGATION_MODE": "shadow", "TYPESAFE_API_KEY": "private-key",
        "SAGE_JEV_RELEVANCE_SCORE_THRESHOLD": "1.5", "SAGE_JEV_RELEVANCE_CONFIDENCE_THRESHOLD": ".3"})
    assert (enabled.score_threshold, enabled.confidence_threshold) == (1.5, .3)
