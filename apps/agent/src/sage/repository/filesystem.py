"""Safe repository paths and deterministic UTF-8 file operations."""
...
def read_file(
    ...
    *,
    ...
    content = _read_utf8(resolved, path, binary_action="read")
...
def replace_text(
    ...
    *,
    ...
    content = _read_utf8(target, path, binary_action="edited")
...
def _read_utf8(path: Path, display_path: str, *, binary_action: str) -> str:
    try:
        data = path.read_bytes()
    except OSError as error:
        raise RepositoryError(f"Unable to read repository file: {display_path}") from error
    if b"\x00" in data[:8192]:
        raise RepositoryError(
            f"Binary repository files cannot be {binary_action}: {display_path}"
        )
    try:
