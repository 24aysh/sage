"""SQLite storage for the repository retrieval index code graph."""
...
from sage.harness.retrieval.migrations import SCHEMA_VERSION, apply_migrations
from sage.harness.retrieval.parsing import EdgeRecord, NodeRecord, ParsedFile
from sage.harness.retrieval.communities import community_groups
from sage.harness.retrieval.resolution import resolve_symbol
from sage.harness.retrieval.bindings import infer_composition
from sage.harness.retrieval.tsconfig import config_aliases
...
class GraphStore:
    ...
    def __init__(self, path: Path, *, read_only: bool = False) -> None:
        ...
        if not read_only:
            ...
            if current is None:
                ...
                raise ValueError(
                    f"Unsupported repository retrieval index schema {current}; expected {SCHEMA_VERSION}."
                )
    ...
    def _validate_schema(self) -> None:
        ...
        if missing:
            raise ValueError(
                "Invalid repository retrieval index database; missing tables: "
                + ", ".join(sorted(missing))
            )
        version = self.get_metadata("schema_version")
        if version is None or int(version) != SCHEMA_VERSION:
            raise ValueError(
                f"Unsupported repository retrieval index schema {version or 'unknown'}; "
    ...
    def _resolve_edge_targets(self) -> None:
        ...
        self.connection.execute("DELETE FROM nodes WHERE kind IN ('Event','ConfigKey') AND json_extract(extra_json,'$.derived_fact')=1")
        rows = self.connection.execute(
        ...
        for row in virtual:
            ...
            self._insert_node(NodeRecord(kind, name, target, str(row["file_path"]),
                ...
                str(row["file_path"]), extra={"derived_fact": True}),
