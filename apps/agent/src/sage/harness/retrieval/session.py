"""Run-scoped Repository retrieval index binding and bounded native-tool usage evidence."""
...
from sage.harness.retrieval.context import render_structural_context
...
class RetrievalSession:
    """Validated base-snapshot retrieval available during one Solver run."""
    ...
    _tool_calls: list[RetrievalToolCallRecord] = field(default_factory=list)
    enrichment_enabled: bool = True
    _closed: bool = False
    _enriched: set[str] = field(default_factory=set)
    _enrichment_records: list[RetrievalToolCallRecord] = field(default_factory=list)
    ...
    def tools_enabled(self) -> bool:
        return not self._closed and self.retrieval.status is RetrievalStatus.USED
    ...
    def initial_context(self) -> str | None:
        if self.retrieval.status is RetrievalStatus.USED:
            return self.retrieval.context
        return None

    @property
    def tool_calls(self) -> tuple[RetrievalToolCallRecord, ...]:
    ...
    def artifact(self) -> RetrievalRunArtifact:
        """Snapshot current retrieval evidence for atomic artifact persistence."""
        ...
        return RetrievalRunArtifact(
            ...
            requested_index_file=self.requested_index_file,
            resolved_index_file=self.index_file,
    ...
    def enrich(
        ...
        try:
            rows = self.service.enrich_repository_read(
                repo_root=self.repo_root, index_file=self.index_file,
        ...
        except (RetrievalQueryError, sqlite3.Error, OSError, ValueError):
            status = "unavailable"
        rendered = output + suffix if selected else ""
        self._enrichment_chars += len(rendered)
        self._session_chars += len(rendered)
        self._enrichment_records.append(RetrievalToolCallRecord(
        ...
        logger.info("Repository retrieval enrichment: %s; tool=%s; symbols=%d", status, tool_name, len(selected))
        return rendered
...
def unavailable_retrieval_artifact(
    *,
    requested_index_file: Path,
    ...
    return RetrievalRunArtifact(
        requested_index_file=requested_index_file,
        resolved_index_file=resolved_index_file,
