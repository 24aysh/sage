"""Current SQLite schema for the rebuildable repository retrieval index."""
...
# A repository index is derived entirely from committed source. New databases
# start directly at the current schema instead of replaying obsolete storage
# history. Older partial schemas must be rebuilt; v4 indexes remain readable.
MIGRATIONS = (
    Migration(
...
def apply_migrations(connection: sqlite3.Connection) -> None:
    """Create a current index or validate an already-current one."""

    current = int(connection.execute("PRAGMA user_version").fetchone()[0])
    if current not in {0, SCHEMA_VERSION}:
        raise ValueError(
            f"Repository index schema {current} is obsolete; move or remove the derived "
            f"index, then rebuild with schema {SCHEMA_VERSION}."
        )
