...
from sage.harness.retrieval.queries import symbol_candidates
...
def refactor_preview(store: GraphStore, mode: str, old_name: str | None, new_name: str | None, limit: int) -> dict:
    if mode == "dead_code":
        ...
        if not old_name or not new_name or not re.fullmatch(r"[A-Za-z_$][\w$]*", new_name):
            raise RetrievalQueryError("Rename preview requires a symbol and a valid new identifier.")
        ...
        raise RetrievalQueryError("Refactor mode must be rename, dead_code, or suggest.")
