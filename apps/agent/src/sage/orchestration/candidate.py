"""Deterministic candidate helpers for the sequential solve workflow."""
...
def create_candidate_snapshot(
    *,
    ...
    if len(diff) > max_diff_chars:
        raise AgentRuntimeError("Candidate diff exceeds the configured context cap.")
...
def ensure_candidate_unchanged(
    *,
    ...
    """Reject candidate drift after independent review."""

    if repository.get_head_sha() != base_sha:
        raise AgentRuntimeError("Candidate HEAD changed after independent review.")
    current = repository.get_complete_diff()
    if hashlib.sha256(current.encode("utf-8")).hexdigest() != diff_digest:
