...
under `apps/agent/src/issue_agent/runtimes/langgraph/`. Deterministic tests,
dependency audits, graph rendering, CLI checks, and sandbox smoke testing were
completed on 2026-08-18. A paid live OpenAI parity solve remains a manual
release check and is documented in [`06_V0.1_testing.md`](06_V0.1_testing.md).
...
- an HTTP API, database, queue, remote worker, or web application changes;
...
| --- | --- | --- |
| CLI command | `issue-agent solve` | Unchanged |
...
| Runtime limit | `ISSUE_AGENT_MAX_TURNS`, default `30` | Unchanged name, default, and lower-bound validation |
...
- `issue_agent.domain.runtime.AgentRuntime`
- `issue_agent.domain.runtime.RuntimeContext`
- `issue_agent.domain.results.AgentFinalOutput`
- `issue_agent.domain.results.SolveResult`
- `issue_agent.workflow.solve.solve_issue`
- `issue_agent.repository.RepositoryTools`
- every implementation under `issue_agent.repository`
- `issue_agent.sandbox.Sandbox` and `DockerSandbox`
...
Set `strict=False` explicitly. Current `langchain-openai` releases default to
strict tool schemas when `response_format` is provided, but several existing
tool parameters are optional and have defaults. Strict tool schemas accept only
a narrower JSON Schema subset and would change those schemas. The selected
package version must be tested with all six tools and the structured response
format. Because provider-side strictness is disabled, project-owned Pydantic
validation in `finalize` is authoritative and a missing/invalid parsed response
must fail clearly.
...
def build_graph(*, model: BaseChatModel, tools: list[BaseTool], max_turns: int):
    ...
    return builder.compile(name="issue_agent_v0_1")
```
...
`ISSUE_AGENT_MAX_TURNS` is enforced by `model_turns`; do not substitute
...
`ISSUE_AGENT_MAX_TURNS=N` means at most `N` executions of the `agent` node.
...
### `issue_agent/runtimes/langgraph/prompt.py`
...
### `issue_agent/runtimes/langgraph/tools.py`
...
### `issue_agent/runtimes/langgraph/graph.py`
...
### `issue_agent/runtimes/langgraph/runtime.py`
...
### `issue_agent/runtimes/langgraph/__init__.py`
...
### Delete `issue_agent/runtimes/openai_agents/`
...
uv run --project apps/agent issue-agent solve --help
...
- the CLI prints `IssueAgent V0` unless product naming is separately approved;
...
- Set `ISSUE_AGENT_MAX_TURNS=1` for a task that needs inspection and confirm a
