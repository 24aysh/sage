# Sage V0.1 LangGraph As-Built Implementation Specification

## Document status

> **Status:** Historical as-built record. Retained for design history; use
...
Frontend implementation is intentionally outside this document except for the
Sage product-name update. The main scope is the Python agent, LangGraph runtime,
deterministic repository and sandbox boundaries, developer tooling, and
supporting documentation.

### Sage naming revision

The project name is Sage across tracked product and implementation surfaces:

| Surface | Current identifier |
| --- | --- |
| Product and CLI banner | `Sage` |
| Distribution and CLI command | `sage` |
| Python package | `sage` |
| Configuration prefix | `SAGE_` |
| Default artifacts | `.sage/runs` |
| Docker image and containers | `sage-sandbox:v0`, `sage-<run-id>` |
| LangGraph identity | `sage_v0_1` |

This is a naming-only migration. Solve arguments, configuration value
semantics, graph routing, tool schemas, model behavior, safety controls,
artifact formats, and exit codes are unchanged. Existing local run records in
the development checkout were moved intact into `.sage/runs`; no run data was
deleted.
...
| --- | --- |
| Request and prepared-run models | `sage.domain.requests` |
| Provider-neutral runtime contract | `sage.domain.runtime.AgentRuntime` |
| Structured agent result | `sage.domain.results.AgentFinalOutput` |
| Authoritative solve result | `sage.domain.results.SolveResult` |
| Central environment configuration | `sage.config.Settings` |
| Run workspace preparation | `sage.repository.workspace.prepare_run` |
| Repository inspection and mutation | `sage.repository.RepositoryTools` |
| Docker isolation | `sage.sandbox.docker.DockerSandbox` |
| Run artifact persistence | `sage.artifacts.ArtifactStore` |
| Solve lifecycle and cleanup | `sage.workflow.solve_issue` |
| CLI request validation and rendering | `sage.cli` |
...
```text
apps/agent/src/sage/runtimes/langgraph/
  __init__.py
  graph.py
  prompt.py
  runtime.py
...
`SAGE_MAX_TURNS` controls model decisions, not raw graph steps. One
...
The CLI constructs `LangGraphRuntime(settings)` and exposes the Sage command
with the same solve arguments and meanings:
...
The printed product label is `Sage V0`; the internal runtime milestone is
V0.1.
...
| `OPENAI_MODEL` | `gpt-5.4-mini` | Host-side chat model name. |
| `SAGE_MAX_TURNS` | `30` | Integer greater than or equal to one. |
| `SAGE_RUNS_DIR` | `.sage/runs` | Host artifact and clone root. |
| `SAGE_SANDBOX_IMAGE` | `sage-sandbox:v0` | Docker image used for repository commands. |
| `SAGE_COMMAND_TIMEOUT_SECONDS` | `60` | Positive command timeout ceiling. |
| `SAGE_MAX_TOOL_OUTPUT_CHARS` | `12000` | Tool-output bound, minimum `1000`. |
...
exercise the real graph without network or live model use.
...
| --- | --- |
| Full backend pytest suite | `68 passed` |
...
| Sage package import, CLI help, solve help, and Make help | Passed. |
| Sage identifier audit | No active previous-name matches in tracked project files. |
...
| `apps/agent/src/sage/runtimes/langgraph/__init__.py` | Exports `LangGraphRuntime`. |
| `apps/agent/src/sage/runtimes/langgraph/prompt.py` | Owns static instructions and initial human message construction. |
| `apps/agent/src/sage/runtimes/langgraph/tools.py` | Adapts the six repository capabilities to LangChain tools. |
| `apps/agent/src/sage/runtimes/langgraph/graph.py` | Owns state schemas, nodes, routing, topology, and graph compilation. |
| `apps/agent/src/sage/runtimes/langgraph/runtime.py` | Owns provider construction, binding, graph invocation, limits, and exception translation. |
| `apps/agent/src/sage/cli.py` | Selects `LangGraphRuntime` behind the unchanged workflow contract. |
| `apps/agent/src/sage/domain/runtime.py` | Describes the active provider-neutral runtime contract. |
...
| `apps/agent/tests/test_cli.py` | Verifies the Sage CLI program name and result banner. |
| `Makefile` | Adds one-command setup/solve and graph inspection. |
| `apps/web/src/app/` and `apps/web/src/components/hero.tsx` | Use Sage in metadata and visible product copy. |
...
- [x] A live solve completed in the authorized local environment.
